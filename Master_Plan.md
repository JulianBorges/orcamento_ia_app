...
- [x] **2.1 Migração do Backend (Cloud Run):**
  - [x] Empacotar a API FastAPI Python em um container Docker e migrá-la da Vercel para uma plataforma de longa duração (Google Cloud Run), eliminando a barreira de timeout de 10s–60s.

- [x] **2.2 Filas de Processamento e Streaming em Tempo Real:**
  - [x] Implementar tarefas em segundo plano para planilhas extensas usando Celery + Redis ou Inngest.
  - [x] Adicionar Server-Sent Events (SSE) para atualizar a tabela na tela do usuário em tempo real, linha a linha (estilo streaming).

- [x] **2.3 Cache Semântico Global (Redis):**
  - [x] Criar camada de cache com Hash SHA-256 e TTL de 7 a 15 dias para memorizar pesquisas por itens comuns (ex: "Cimento CP II").
  - [x] Validar a redução de latência para milissegundos e o corte de custos repetitivos de chamadas à OpenAI.
...
- [x] **3.1 Algoritmo RRF (Reciprocal Rank Fusion):**
  - [x] Substituir regras manuais de bônus (+0.5) pelo algoritmo matemático RRF para fundir de forma estável o ranking do Pinecone (Busca Semântica) e do SQLite (Busca Lexical).

- [x] **3.2 Suporte a Insumos Avulsos (SINAPI-I vs SINAPI-C):**
  - [x] Manter isolamento estrito: a triagem principal pesquisa apenas em Composições (SINAPI-C).
  - [x] A base de Insumos (SINAPI-I) só é consultada via Function Calling ou para itens categorizados exclusivamente como material avulso.

- [x] **3.3 Over-fetching, Filtros Rígidos e JSON Schema:**
  - [x] Resgatar Top 20 dos bancos, aplicar Reranking e enviar apenas o Top 5 final para a LLM.
  - [ NÃO ] Aplicar filtros obrigatórios de Estado (UF) e Desoneração na query antes da busca vetorial.
  - [x] Substituir parsing via Regex pelo formato Structured Outputs (JSON Schema) da OpenAI.

- [x] **3.4 Engenharia de Prompt Especializada:**
  - [ NÃO ] Injetar lógica de BDI diferenciado (Súmula 2622 do TCU) para fornecimento de materiais e equipamentos específicos.
  - [x] Aplicar tolerâncias paramétricas (ex: margem rígida para aço, elástica para tubulações).
